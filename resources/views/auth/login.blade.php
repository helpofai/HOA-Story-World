...
@push('styles')
    @vite(['resources/css/auth-animation.css'])
@endpush

@push('scripts')
    @vite(['resources/js/auth-animation.js'])
@endpush

@section('content')
<div class="auth-animation-container">
    <div class="auth-form-container">
        <!-- Branding inside animated area -->
        <div class="text-center mb-6">
            <a href="/" class="text-3xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-pink-600">
                Story.
            </a>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" id="login-form">
            ...
            <!-- Email -->
            <div class="form-row">
                <input type="email" id="email" name="email" placeholder="email address" value="{{ old('email') }}" required autofocus autocomplete="username">
            </div>
            ...
            <div class="form-row">
                <label class="flex items-center">
                    ...
                    <span class="text-xs text-gray-500 ml-2 font-bold uppercase tracking-wider">Stay logged in</span>
                </label>
            </div>
            ...
            <div class="mt-6 flex flex-col items-center gap-2">
                @if (Route::has('password.request'))
                    <a class="text-[10px] font-bold text-gray-400 uppercase hover:text-blue-600 transition-colors" href="{{ route('password.request') }}">
                        Forgot password?
                    </a>
                @endif
                <a class="text-[10px] font-bold text-gray-400 uppercase hover:text-pink-600 transition-colors" href="{{ route('register') }}">
                    New here? Register
                </a>
            </div>
        </form>
    ...
    <svg class="auth-animation-svg" viewBox="0 0 1000 1000" stroke-linecap="round" stroke-linejoin="round">
