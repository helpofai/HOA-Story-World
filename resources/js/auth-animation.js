document.addEventListener('DOMContentLoaded', () => {
    const containerEl = document.querySelector('.auth-animation-container');
    if (!containerEl) return;

    const checkboxEl = document.querySelector('.auth-form-container input[type="checkbox"]');
    const nameEl = document.querySelector('.auth-form-container input[name="name"]');
    const emailEl = document.querySelector('.auth-form-container input[name="email"]');
    const passwordEl = document.querySelector('.auth-form-container input[name="password"]');
    const submitBtn = document.querySelector('.auth-form-container input[type="submit"]');
    ...
    const grabbingHands = document.querySelectorAll('.grabbing-hand');
    ...
    function triggerGearAnimation(isValid) {
        if (isValid) {
            gearsTls.forEach(tl => {
                if (tl.paused()) {
                    tl.play();
    ...
    // Adapt logic based on which fields are present (Login vs Register)
    const primaryInput = nameEl || emailEl;
    const secondaryInput = nameEl ? emailEl : passwordEl;

    if (primaryInput) {
        primaryInput.addEventListener('input', () => {
            const isValid = primaryInput.value.length > 3;
            if (isValid) primaryInput.classList.add('valid');
            else primaryInput.classList.remove('valid');
            triggerGearAnimation(isValid);
        });
        // Init check
        if (primaryInput.value.length > 3) {
            primaryInput.classList.add('valid');
            triggerGearAnimation(true);
        }
    }
    ...
    if (secondaryInput) {
        secondaryInput.addEventListener('input', () => {
            ...
            const isValid = secondaryInput.type === 'email' 
                ? emailRegex.test(secondaryInput.value)
                : secondaryInput.value.length > 5;

            if (isValid) {
                secondaryInput.classList.add('valid');
                emailTl.play();
            } else {
                secondaryInput.classList.remove('valid');
                emailTl.reverse();
            }
        });
        // Init check
        if (secondaryInput.value.length > 5) {
             emailTl.play();
             secondaryInput.classList.add('valid');
        }
    }
    ...
    function createEmailTl() {
        ...
        const tl = gsap.timeline({
            ...
            onUpdate: () => updateSpiralPath(14, 14, 45, 17, 200, spiralTurnsNumber * spiralProgress.v),
        })
        ...
        tl.fromTo(state, { handClosed: false }, { duration: .01, handClosed: true }, ">")
          .to(grabbingHands, { duration: fingersTimeDelta * 5, x: "+=20" }, hammerTimeStart + fingersDelay);
    ...
    function createGearsTimelines() {
        ...
        const params = {
            ...
            startPos: {x: 634, y: 389}, speed: .2
        };
        const data = [{ angle: 0, teethNumber: 10, hasHole: true }, 
                ...
                tl.eventCallback("onUpdate", () => {
                    ...
                    if (deltaY > 8) {
                        ...
                        if (!state.sumbitBtnOnPlace) {
                            ...
                            state.sumbitBtnTextOpacity = Math.max(0, Math.min(1, Math.pow(state.sumbitBtnTextOpacity, 2)));
                        }
    ...
    function createPullingTimeline(isFixed, BtnPulled) {
        ...
        if (isFixed && BtnPulled) {
            tl.to(state, { pullProgress: 1 }, 0)
              ...
              .to(state, { duration: .1, sumbitBtnOnPlace: true }, .9)
            ...
            tl.to(state, { pullProgress: 0 }, 0)
              ...
              .to(state, { duration: .1, sumbitBtnOnPlace: false }, 0)
